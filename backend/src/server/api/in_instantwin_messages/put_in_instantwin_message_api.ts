...
import { MessageUpdateRequest } from './types/message_request';
import { MessageDetailResponse } from './types/message_response';

export const putInInstantwinMessageApi = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const messageData: MessageUpdateRequest = req.body;

    const message = await InInstantwinMessageService.updateMessage(numericId, messageData);
    ...
    ResponseHelper.success(res, response, MESSAGE_CONSTANTS.MESSAGES.MESSAGE_UPDATED);
  } catch (error) {
    console.error('Error in putInInstantwinMessageApi:', error);
    ...
    if (error instanceof Error) {
      if (error.message.startsWith(MESSAGE_CONSTANTS.ERROR_CODES.VALIDATION_ERROR)) {
        ...
        ResponseHelper.internalServerError(res, 'メッセージの更新に失敗しました');
      }
    } else {
      ResponseHelper.internalServerError(res, 'メッセージの更新に失敗しました');
    }
